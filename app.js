...
function switchPage(pageId, element) {
  ...
  if (titles[pageId]) {
    ...
    if (subTitleEl) subTitleEl.innerText = titles[pageId][1];
  }
}

function renderKPIs() {
  const totalItems = dataProvinsi.length;
  if (!totalItems) return;

  const avgRTP = (dataProvinsi.reduce((acc, curr) => acc + curr.rtp, 0) / totalItems).toFixed(2);
  const avgIKRP = (dataProvinsi.reduce((acc, curr) => acc + curr.ikrp, 0) / totalItems).toFixed(1);
  ...
  const ikrpEl = document.getElementById('kpi-ikrp');
  const prodEl = document.getElementById('kpi-produksi');
  const kritisEl = document.getElementById('kpi-kritis');

  if (rtpEl) rtpEl.innerText = `${avgRTP}x`;
  if (ikrpEl) ikrpEl.innerText = avgIKRP;
  if (prodEl) prodEl.innerText = `${totalProduksi} Jt Ton`;
  if (kritisEl) kritisEl.innerText = `${p1Count} Wilayah`;
}

// Render Tabel secara Aman (Mencegah XSS)
function renderOverviewTable(data) {
  ...
  data.forEach(item => {
    ...
    tr.innerHTML = `
      ...
      <td></td>
      <td></td>
      <td><span class="badge ${getStatusBadgeClass(item.status)}"></span></td>
      <td>
        <button class="btn btn-secondary btn-sm"><i class="fa-solid fa-eye"></i> Detail</button>
      </td>
    `;

    // Pengisian konten secara aman
    const cells = tr.querySelectorAll('td');
    cells[0].textContent = item.kode;
    cells[1].textContent = item.provinsi;
    cells[2].textContent = item.pulau;
    
    cells[3].textContent = item.rtp.toFixed(2);
    cells[3].style.color = item.rtp > 1.3 ? '#ef4444' : '#1e293b';

    cells[4].textContent = item.produksi.toLocaleString('id-ID');

    cells[5].textContent = `${item.cagr > 0 ? '+' : ''}${item.cagr.toFixed(2)}%`;
    cells[5].style.color = item.cagr < 0 ? '#ef4444' : '#10b981';

    cells[6].textContent = item.ikrp.toFixed(1);
    cells[7].textContent = `${item.kontribusi.toFixed(2)}%`;

    const badgeSpan = cells[8].querySelector('span');
    if (badgeSpan) badgeSpan.textContent = item.status;

    const detailBtn = cells[9].querySelector('button');
    if (detailBtn) detailBtn.onclick = () => openModal(item.kode);
...
function applyFilters() {
  ...
  const filterPulau = document.getElementById('filterPulau');
  const filterStatus = document.getElementById('filterStatus');

  const query = searchInput ? searchInput.value.toLowerCase() : '';
  const pulau = filterPulau ? filterPulau.value : 'ALL';
...
function openModal(kode) {
  ...
  const modalRtp = document.getElementById('modalRtp');
  const modalProduksi = document.getElementById('modalProduksi');
  const modalCagr = document.getElementById('modalCagr');
  const modalIkrp = document.getElementById('modalIkrp');
  const modalRekomendasi = document.getElementById('modalRekomendasi');

  if (modalTitle) modalTitle.innerText = data.provinsi;
  if (modalBadge) {
    modalBadge.innerText = data.status;
    modalBadge.className = `badge ${getStatusBadgeClass(data.status)}`;
  }

  if (modalRtp) modalRtp.innerText = `${data.rtp.toFixed(2)}x`;
  if (modalProduksi) modalProduksi.innerText = `${data.produksi.toLocaleString('id-ID')} Ribu Ton`;
  if (modalCagr) modalCagr.innerText = `${data.cagr > 0 ? '+' : ''}${data.cagr.toFixed(2)}%`;
  if (modalIkrp) modalIkrp.innerText = data.ikrp.toFixed(1);
  ...
  if (modalRekomendasi) {
    modalRekomendasi.innerText = reks[data.status] || 'Penguatan kapasitas kelembagaan tani dan proteksi pasokan lokal.';
  }
...
function exportData(format) {
  if (format === 'json') {
    ...
    let csvContent = "\uFEFFKode,Provinsi,Pulau,RTP,Produksi_RibuTon,CAGR,IKRP,Kontribusi_Persen,Status\n";
...
function initRtpChart() {
  ...
  // Mencegah kebocoran memori Chart.js
  if (rtpChartInstance) {
...
// Chart 2: Scatter / Bubble Chart Matrix
